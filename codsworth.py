...
# Saves user's info to a json file
    ...
    # User's mood
    match = re.search(r"\b(I[' ]?m|I am|I feel|I'm feeling)\s+(feeling\s+)?(happy|sad|angry|tired|excited|bored|lonely|anxious|fine|okay|good|great|terrible)\b", user_input, re.IGNORECASE)
    if match:
        mood = match.group(3).lower()
        memory["mood"] = mood
        save_memory(memory)
